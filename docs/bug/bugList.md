...
14. **[源码已修复，待发布 2026-09-03] 部分手机网络无法连接 GitHub 官方 APK 域名**
   ...
   - 修复：按 GitHub API 资产、GitHub 官方资源、`gh-proxy.com`、`gh.3w.pm` 顺序尝试；所有来源继续强制校验文件大小和 SHA-256。API 完整 APK 与两个镜像分段请求已验证，尚未发布到用户设备。验收见 [Stage 463](../stages/stage-463-update-download-fallback.md)。

13. **[源码已修复，待发布 2026-09-01] 收集箱弹窗空白过大且操作层级混乱**
   - 现象：空收集箱仍显示大面积空内容区，“收下文字/链接”“关闭”“整理工作台”以系统文字按钮散落在右侧；顶部入口在深色主题下对比度不足。
   - 根因：收集箱直接使用系统默认 `setItems` 对话框，没有为空状态、记录区及主次操作设计独立布局。
   - 修复：新增收集箱卡片布局和明确空状态；主操作改为全宽实心按钮，工作台改为描边次按钮，关闭置于标题栏；顶部入口增加语义图标和固定白色文字。
   - 验证：Android 14 实际截图、Debug 构建及原 selfcheck 6/6 通过，见 [Stage 461](../stages/stage-461-workspace-ui-polish.md)。

12. **[源码已修复，待发布 2026-09-01] “我的”页顶部工作台入口排版错乱**
   - 现象：窄屏设备上三个胶囊按钮间距不足，“找回与关联”被挤成两行，按钮高度不一致。
   - 根因：入口由代码运行时插入，等宽按钮仍继承 MaterialButton 默认 inset/padding，且没有单行和字号适配约束。
   - 修复：入口迁入 `fragment_profile.xml`；设置 48dp 触控高度、8dp 间距、单行及 12sp–14sp 自动字号，点击行为不变。
   - 验证：Android 14、1080×2400 AVD 截图复核通过；Debug 构建成功；原 selfcheck 6/6、Android 69 项 0 失败。证据见 [Stage 460](../stages/stage-460-profile-layout-fix.md)。

11. **[源码已修复，待发布 2026-09-01] v4.3.8 点击移动端更新后无明显反馈**
   - 现象：发现新版本后点击更新，版本弹窗关闭；受限网络下应用内 GitHub 直连没有及时进入安装器，用户无法判断是否开始下载。
   - 根因：未缓存 APK 的主按钮直接进入应用内下载线程，浏览器入口仅在下载最终失败后出现。
   - 修复：主按钮立即打开系统 chooser/浏览器的 GitHub 官方 APK，并在启动前显示提示；应用内摘要校验下载保留为明确标注的备用入口；浏览器不可用时显示错误。
   - 验证：新增官方 URL 选择回归；原 selfcheck 6/6、Android 69 项 0 失败。当前正式版仍为 v4.3.8，需明确授权发布下一 patch 后才能到达用户设备。

10. **[已修复 2026-09-01] 4.3.6 与本地整合存在字段及安全回退**
   - 远端转卖和订阅调用使用 `retiredDate/subStartDate`，本地持久化使用 `retiredAt/purchaseDate`，直接整合无法编译；统一到已持久化字段，防止更新时间丢失。
   - 远端 DEX 验签与 ZIP 边界检查缺失；恢复检查并增加真实 RSA 负向回归，未配置补丁公钥时继续拒绝 DEX。
   - 双方测试冲突保留本地准确的 `take(20)` 预览断言；远端硬编码预览字符数与 20 不一致。远端新增桌面关联与合并测试保留。
   - 桌面下载未记住已核对 ETag，恢复后仍不能条件上传；补记下载响应 ETag，并禁止 PUT/GET 跟随重定向，避免凭据发送到未验证地址。
   - 升级数据兼容：4.3.6 的工具、绿植、宠物、饮品使用另一组 SharedPreferences 键，且日期/进度等字段存在别名。增加一次性原子迁移，保留原始键与未知字段；饮品小数余量不能按整数截断。
   - 验证：原自检 6/6，三端单测与 API 34 迁移、公网双端及跨端恢复通过，见 [Stage 457](../stages/stage-457-report.md)。修复在源码中，不表示已替换历史发布附件。
