...
- [ ] AC-1: 实现 `ScreenshotWatcherHelper.kt`，支持在 Android 8.0~14 上可靠监听系统截图事件并去重；
- [ ] AC-2: 实现 `ScreenshotOcrProcessor.kt`，集成 ML Kit 离线识别，自动将截图生成 `ClippingRecord` 存入知识库；
- [ ] AC-3: 实现 `WebClipperEngine.kt`，支持 URL 纯净正文提取与 Markdown 转换；
- [ ] AC-4: 在 `SettingsStore` / UI 中增加「📸 截图无感自动收纳」配置开关；
- [ ] AC-5: 编写单元测试验证网页正文提取与智能打标逻辑；
- [ ] AC-6: 门禁自检 `tools/selfcheck.ps1` 100% 绿灯，架构文档与 TODO 同步更新。
