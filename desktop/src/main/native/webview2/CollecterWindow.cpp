...
bool isAllowedUrl(const std::wstring& url) {
    return url == L"about:blank" || url == L"http://127.0.0.1:8848" ||
        startsWith(url, L"http://127.0.0.1:8848/") || url == L"http://localhost:8848" ||
        startsWith(url, L"http://localhost:8848/");
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TIMER:
        if (wParam == ID_PROBE_TIMER) {
            ...
            if (probeBackendReady()) {
                ...
                KillTimer(hwnd, ID_PROBE_TIMER);
                if (!smokeTest) MessageBoxW(hwnd, L"后端服务启动超时，请检查服务运行日志。", L"Collecter 启动失败", MB_OK | MB_ICONERROR);
                DestroyWindow(hwnd); PostQuitMessage(8);
            }
            return 0;
        }
        break;
    ...
    case WM_COMMAND:
        ...
        case ID_TRAY_CHECK_UPDATE:
            MessageBoxW(hwnd, L"此版本未配置自动发行检查；请以经过验证的发布清单为准。", L"版本检查", MB_OK | MB_ICONINFORMATION);
            break;
        case ID_TRAY_EXIT:
            removeTrayIcon();
            DestroyWindow(hwnd);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR pCmdLine, int nCmdShow) {
    ...
    smokeTest = pCmdLine && std::wstring(pCmdLine).find(L"--smoke-test") != std::wstring::npos;
    HRESULT comResult = CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED);
    if (FAILED(comResult)) return 2;
    const wchar_t* local = _wgetenv(L"LOCALAPPDATA");
    if (!local) return 3;
    std::filesystem::path profile = std::filesystem::path(local) / L"CollecterStandalone" / L"webview";
    if (const wchar_t* isolated = _wgetenv(L"COLLECTER_DATA_DIR")) profile = std::filesystem::path(isolated) / L"webview";
    std::filesystem::create_directories(profile);
    userDataDirectory = profile.wstring();
    ...
    // 初始化 WebView2
    HRESULT environmentResult = CreateCoreWebView2EnvironmentWithOptions(nullptr, userDataDirectory.c_str(), nullptr,
        Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
            [](HRESULT result, ICoreWebView2Environment* env) -> HRESULT {
                if (FAILED(result) || !env) {
                    initialisationFailed = true;
                    if (!smokeTest) MessageBoxW(windowHandle, L"WebView2 初始化失败，请安装 Microsoft Edge WebView2 Runtime。", L"Collecter 启动失败", MB_OK | MB_ICONERROR);
                ...
                env->CreateCoreWebView2Controller(windowHandle,
                    Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
                        [](HRESULT res, ICoreWebView2Controller* c) -> HRESULT {
                            if (FAILED(res) || !c) { DestroyWindow(windowHandle); PostQuitMessage(5); return res; }
                            controller = c;
                                    ...
                                    [](ICoreWebView2*, ICoreWebView2NavigationStartingEventArgs* args) -> HRESULT {
                                        ...
                                        if (uri) {
                                            if (!isAllowedUrl(uri)) {
                                                ...
                                                if (startsWith(uri, L"https://") || startsWith(uri, L"http://")) {
                            ...
                            webview->add_NavigationCompleted(
                                Callback<ICoreWebView2NavigationCompletedEventHandler>(
                                    [](ICoreWebView2*, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
                                        ...
                                        if (!showingLoadingPage && ok) {
                                            navigationSucceeded = true;
                                            if (smokeTest) { DestroyWindow(windowHandle); PostQuitMessage(0); }
                                        }
                                        return S_OK;
                                    }).Get(), nullptr);
    ...
    if (FAILED(environmentResult)) {
        if (!smokeTest) MessageBoxW(windowHandle, L"无法创建 WebView2 环境。", L"Collecter 启动失败", MB_OK | MB_ICONERROR);
        DestroyWindow(windowHandle);
        CoUninitialize();
        return 6;
    }
    ...
    controller.Reset(); webview.Reset(); sharedEnvironment.Reset();
    CoUninitialize();
    if (smokeTest && !navigationSucceeded) return 7;
