...
class DesktopDataStore(customDataDir: File? = null) {
    ...
    fun loadAll(): List<Entry> {
        ...
        return try {
            ...
            inMemoryIdeas = parseList(root.optJSONArray("ideas")) { io ->
                IdeaRecord(
                    id = io.optString("id", UUID.randomUUID().toString()),
                    content = io.optString("content", ""),
                    tags = parseStringList(io.optJSONArray("tags")),
                    moodEmoji = io.optString("moodEmoji", io.optString("emoji", "💡")),
                    isPinned = io.optBoolean("isPinned", io.optBoolean("pinned", false)),
                    colorHex = io.optString("colorHex", "#10B981"),
                    linkedAssetIds = parseStringList(io.optJSONArray("linkedAssetIds")),
                    createdAt = io.optLong("createdAt", io.optLong("created_at", System.currentTimeMillis())),
                    updatedAt = io.optLong("updatedAt", io.optLong("updated_at", System.currentTimeMillis()))
                )
            }

            inMemoryClippings = parseList(root.optJSONArray("clippings")) { co ->
                ClippingRecord(
                    id = co.optString("id", UUID.randomUUID().toString()),
                    title = co.optString("title", ""),
                    originalUrl = co.optString("originalUrl", co.optString("url", "")),
                    sourcePlatform = co.optString("sourcePlatform", co.optString("platform", "web")),
                    fullMarkdown = co.optString("fullMarkdown", co.optString("markdown", "")),
                    ocrRawText = co.optString("ocrRawText", co.optString("ocr", "")),
                    localImagePaths = parseStringList(co.optJSONArray("localImagePaths")),
                    summary = co.optString("summary", ""),
                    tags = parseStringList(co.optJSONArray("tags")),
                    linkedAssetIds = parseStringList(co.optJSONArray("linkedAssetIds")),
                    capturedAt = co.optLong("capturedAt", co.optLong("captured_at", System.currentTimeMillis()))
                )
            }
    ...
    fun saveAll(list: List<Entry> = inMemoryEntries) {
        ...
        root.put("ideas", serializeList(inMemoryIdeas) { id ->
            JSONObject().put("id", id.id).put("content", id.content)
                .put("tags", JSONArray(id.tags)).put("moodEmoji", id.moodEmoji)
                .put("isPinned", id.isPinned).put("colorHex", id.colorHex)
                .put("linkedAssetIds", JSONArray(id.linkedAssetIds))
                .put("createdAt", id.createdAt).put("updatedAt", id.updatedAt)
        })

        root.put("clippings", serializeList(inMemoryClippings) { cl ->
            JSONObject().put("id", cl.id).put("title", cl.title)
                .put("originalUrl", cl.originalUrl).put("sourcePlatform", cl.sourcePlatform)
                .put("fullMarkdown", cl.fullMarkdown).put("ocrRawText", cl.ocrRawText)
                .put("localImagePaths", JSONArray(cl.localImagePaths))
                .put("summary", cl.summary).put("tags", JSONArray(cl.tags))
                .put("linkedAssetIds", JSONArray(cl.linkedAssetIds))
                .put("capturedAt", cl.capturedAt)
        })
    ...
    // ==================== 💡 灵感想法舱 ====================
    fun getIdeas(): List<IdeaRecord> = inMemoryIdeas.toList()
    fun saveIdeas(list: List<IdeaRecord>) {
        inMemoryIdeas = list.toMutableList()
        saveAll()
    }
    fun addOrUpdateIdea(idea: IdeaRecord) {
        updateInList(inMemoryIdeas, idea, { it.id == idea.id })
    }
    fun deleteIdea(id: String) {
        inMemoryIdeas.removeAll { it.id == id }
        saveAll()
    }
    fun toggleIdeaPin(id: String) {
        val idx = inMemoryIdeas.indexOfFirst { it.id == id }
        if (idx != -1) {
            val old = inMemoryIdeas[idx]
            inMemoryIdeas[idx] = old.copy(isPinned = !old.isPinned, updatedAt = System.currentTimeMillis())
            saveAll()
        }
    }

    // ==================== 📰 智能剪藏知识库 ====================
    fun getClippings(): List<ClippingRecord> = inMemoryClippings.toList()
    fun saveClippings(list: List<ClippingRecord>) {
        inMemoryClippings = list.toMutableList()
        saveAll()
    }
    fun addOrUpdateClipping(record: ClippingRecord) {
        updateInList(inMemoryClippings, record, { it.id == record.id })
    }
    fun deleteClipping(id: String) {
        inMemoryClippings.removeAll { it.id == id }
        saveAll()
    }
