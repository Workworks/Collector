...
object BookVaultDialog {
    ...
    private fun showProgressUpdateDialog(
        ...
        ModernDialogHelper.showInputDialog(
            context = activity,
            title = "📖 阅读进度打卡",
            subtitle = "当前阅读《${record.title}》，全书共 ${record.totalPages} 页",
            hint = "输入当前已读页码",
            defaultValue = "${record.currentPages}",
            emoji = "📖",
            positiveText = "保存进度",
            negativeText = "取消"
        ) { inputStr ->
            val pages = inputStr.toIntOrNull() ?: record.currentPages
    ...
    private fun showLendOrReturnDialog(
        ...
        if (record.isLent()) {
            ...
            ModernDialogHelper.showInputDialog(
                context = activity,
                title = "📤 实体书借出登记",
                subtitle = "将《${record.title}》借出给亲友或同事",
                hint = "输入借阅人姓名 (如：同事小张)",
                defaultValue = "",
                emoji = "📤",
                positiveText = "确认借出",
