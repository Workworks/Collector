...
object UniversalVaultCenterDialog {
    ...
    fun show(activity: Activity, store: DataStore, onDataChanged: () -> Unit = {}) {
        ...
        fun reloadAll() {
            ...
            val tools = store.getToolRecords()
            val plants = store.getPlantRecords()
            val pets = store.getPetRecords()
            val books = store.getBookRecords()
            val beverages = store.getBeverageRecords()
            ...
            binding.tvTileToolSub.text = "${tools.size} 件"
            binding.tvTilePlantSub.text = "${plants.size} 盆"
            binding.tvTilePetSub.text = "${pets.size} 只"
            binding.tvTileBookSub.text = "${books.size} 本"
            binding.tvTileBeverageSub.text = "${beverages.size} 项"

            val totalItems = vouchers.count { !it.isUsed } + docs.size + medicines.size + foods.size + honors.size + wardrobes.size + emergencies.size + tools.size + plants.size + pets.size + books.size + beverages.size
            ...
            // 药品
            medicines.forEach { m ->
                val desc = if (m.dosage.isNotBlank()) "用法: ${m.dosage}" else m.getCategoryDisplayName()
                if (m.isExpired()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "medicine",
                            ...
                            title = m.name,
                            detail = "$desc · 已于 ${VaultUiHelper.standardDateFormat.format(Date(m.getEffectiveExpiryDate()))} 过期",
                            isExpired = true,
                            statusText = "🔴 严禁服用",
                            onClickAction = {
                    ...
                    expiryList.add(
                        AggregatedExpiryItem(
                            ...
                            vaultDisplayName = "💊 家庭药箱",
                            title = m.name,
                            detail = "$desc · 截止: ${VaultUiHelper.standardDateFormat.format(Date(m.getEffectiveExpiryDate()))}",
                            isExpired = false,
                            statusText = "🟡 30天内到期",
                            onClickAction = {
                                FamilyMedicineDialog.showMedicineVaultDialog(activity, store) { reloadAll(); onDataChanged() }
            ...
            // 工具与设备维保
            tools.filter { it.maintenanceIntervalDays > 0 }.forEach { t ->
                if (t.isMaintenanceDue()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "tool",
                            vaultDisplayName = "🔧 工具维保",
                            title = t.name,
                            detail = "${t.location.ifBlank { "工具五金舱" }} · 维保/更换已逾期",
                            isExpired = true,
                            statusText = "🔴 维保已逾期",
                            onClickAction = {
                                ToolMaintenanceDialog.show(activity, store) { reloadAll(); onDataChanged() }
                            }
                        )
                    )
                } else if (t.isMaintenanceDueSoon()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "tool",
                            vaultDisplayName = "🔧 工具维保",
                            title = t.name,
                            detail = "${t.location.ifBlank { "工具五金舱" }} · 下次排期: ${VaultUiHelper.standardDateFormat.format(Date(t.getNextMaintenanceDate()))}",
                            isExpired = false,
                            statusText = "🟡 15天内需维保",
                            onClickAction = {
                                ToolMaintenanceDialog.show(activity, store) { reloadAll(); onDataChanged() }
            ...
            plants.forEach { p ->
                if (p.isWaterDue()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "plant",
                            vaultDisplayName = "🪴 绿植水肥",
                            title = p.name,
                            detail = "${p.location.ifBlank { "在养绿植" }} · 需及时补充水分",
                            isExpired = true,
                            statusText = "💧 需浇水",
                            onClickAction = {
                                PlantCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
                            }
                        )
                    )
                } else if (p.isFertilizeDue()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "plant",
                            vaultDisplayName = "🪴 绿植水肥",
                            title = p.name,
                            detail = "${p.location.ifBlank { "在养绿植" }} · 周期施肥已到期",
                            isExpired = false,
                            statusText = "🌿 需施肥",
                            onClickAction = {
                                PlantCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
            ...
            pets.forEach { p ->
                if (p.isVaccineDue()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "pet",
                            vaultDisplayName = "🐾 萌宠健康",
                            title = p.name,
                            detail = "${p.species} · 核心疫苗接种已逾期",
                            isExpired = true,
                            statusText = "💉 需打疫苗",
                            onClickAction = {
                                PetCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
                            }
                        )
                    )
                } else if (p.isDewormDue()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "pet",
                            vaultDisplayName = "🐾 萌宠健康",
                            title = p.name,
                            detail = "${p.species} · 周期驱虫已逾期",
                            isExpired = true,
                            statusText = "💊 需驱虫",
                            onClickAction = {
                                PetCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
                ...
                } else if (p.isDewormDueSoon()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "pet",
                            vaultDisplayName = "🐾 萌宠健康",
                            title = p.name,
                            detail = "${p.species} · 7天内待驱虫",
                            isExpired = false,
                            statusText = "🟡 7天内待驱虫",
                            onClickAction = {
                                PetCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
            ...
            // 书房藏书外借流转
            books.filter { it.isLent() }.forEach { b ->
                expiryList.add(
                    AggregatedExpiryItem(
                        vaultType = "book",
                        vaultDisplayName = "📚 书房藏书",
                        title = "《${b.title}》",
                        detail = "外借给: ${b.borrowerName} · 借阅流转中",
                        isExpired = false,
                        statusText = "📤 外借中",
                        onClickAction = {
                            BookVaultDialog.show(activity, store) { reloadAll(); onDataChanged() }
                        }
                    )
                )
            }
            ...
            beverages.forEach { b ->
                if (b.isOpenExpired()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "beverage",
                            vaultDisplayName = "🍷 茶窖名酿",
                            title = b.name,
                            detail = "已开瓶品鉴 · 超过开封保鲜/赏味期 (${b.openShelfLifeDays}天)",
                            isExpired = true,
                            statusText = "🔴 开封已超期",
                            onClickAction = {
                                BeverageTeaDialog.show(activity, store) { reloadAll(); onDataChanged() }
                            }
                        )
                    )
                } else if (b.isPeakDrinkingNow()) {
                    expiryList.add(
                        AggregatedExpiryItem(
                            vaultType = "beverage",
                            vaultDisplayName = "🍷 茶窖名酿",
                            title = b.name,
                            detail = "${b.vintageYear}年产 (陈化${b.getAgingYears()}年) · 正值最佳适饮熟成黄金期",
                            isExpired = false,
                            statusText = "✨ 适饮黄金期",
                            onClickAction = {
                                BeverageTeaDialog.show(activity, store) { reloadAll(); onDataChanged() }
        ...
        binding.tileToolVault.applyPressScaleAnimation(0.92f)
        binding.tileToolVault.setOnClickListener {
            ToolMaintenanceDialog.show(activity, store) { reloadAll(); onDataChanged() }
        }

        binding.tilePlantVault.applyPressScaleAnimation(0.92f)
        binding.tilePlantVault.setOnClickListener {
            PlantCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
        }

        binding.tilePetVault.applyPressScaleAnimation(0.92f)
        binding.tilePetVault.setOnClickListener {
            PetCareDialog.show(activity, store) { reloadAll(); onDataChanged() }
        }

        binding.tileBookVault.applyPressScaleAnimation(0.92f)
        binding.tileBookVault.setOnClickListener {
            BookVaultDialog.show(activity, store) { reloadAll(); onDataChanged() }
        }

        binding.tileBeverageVault.applyPressScaleAnimation(0.92f)
        binding.tileBeverageVault.setOnClickListener {
            BeverageTeaDialog.show(activity, store) { reloadAll(); onDataChanged() }
        }
