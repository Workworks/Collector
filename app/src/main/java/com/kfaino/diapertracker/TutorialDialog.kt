...
object TutorialDialog {
    ...
    private val TUTORIAL_LIST = listOf(
        ...
        TutorialItem(
            category = "photo",
            icon = "📸",
            title = "拍照发票/小票 OCR 识别与一句话记账",
            summary = "智能多模态免录入，拍照超市小票、外包装或粘贴一句话直接自动填充入库。",
            steps = listOf(
                "在【记一笔】弹窗顶部点击【📷 拍照发票/小票秒录】，拍摄或选取照片，AI 自动提取品名、单价、日期、保质期并自动填充；",
                "点击【💬 一句话智能记账】，粘贴自然语言（如「昨天在山姆买了2箱牛奶单价65保质期到10月」），AI 自动结构化解析并填入表单。"
            ),
            tip = "基于端侧离线 OCR 模型，断网也能极速识别！"
        ),
        TutorialItem(
            category = "sync",
            icon = "🌐",
        ...
        TutorialItem(
            category = "asset",
            icon = "🛒",
            title = "耗材安全库存预警与一键智能采购清单",
            summary = "为生活耗材设定最低安全库存线，告别断货断供尴尬。",
            steps = listOf(
                "在录入或编辑物品时填写【⚠️ 最低安全库存预警线】；",
                "当库存消耗至低于或等于安全线时，首页顶部自动浮现【⚠️ 耗材安全库存告急卡片】；",
                "点击【🛒 一键生成采购单】，自动汇总所有缺货品类与需补差额，格式化复制到剪贴板直接发给微信或购物平台。"
            ),
            tip = "采购单文本已格式化对齐，包含品名、当前存量与推荐补货量！"
        )
    )
