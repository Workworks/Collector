...
object ClippingVaultDialog {
    ...
    private fun showAddClippingDialog(activity: Activity, store: DataStore, onSaved: () -> Unit) {
        ...
        // 网页链接智能抓取行
        val urlRow = LinearLayout(activity).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            val lp = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
        ...
        val btnFetch = Button(activity).apply {
            text = "⚡ 智能抓取"
            textSize = 12f
            setTextColor(Color.WHITE)
            background = GradientDrawable().apply {
        ...
        btnFetch.setOnClickListener {
            ...
            if (url.isEmpty()) {
                Toast.makeText(activity, "请先输入网页链接", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            Toast.makeText(activity, "⚡ 正在智能抓取网页正文...", Toast.LENGTH_SHORT).show()
            WebClipperEngine.clipUrl(url) { result ->
                activity.runOnUiThread {
                    if (result != null) {
                        etTitle.setText(result.title)
                        etContent.setText(result.fullMarkdown)
                        etTags.setText(result.tags.joinToString(" "))
                        Toast.makeText(activity, "✅ 网页已解析，可直接保存入库！", Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(activity, "抓取失败，请检查网址有效性或网络连接", Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
