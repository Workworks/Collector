...
data class MergeReport(
    ...
    fun summary(): String {
        return "增量合并完成：新增资产 ${insertedEntries} 件，更新资产 ${updatedEntries} 件，保留未变 ${preservedEntries} 件，合并分类 ${mergedCategories} 个，合并馆藏/资料 ${mergedVaultItems} 项。$message"
    }
}
...
object LanSyncMergeEngine {
    ...
    fun merge(store: DataStore, incomingJsonStr: String): MergeReport {
        return try {
            val merged = com.kfaino.collecter.core.SnapshotSync.merge(store.exportBackupJson(), incomingJsonStr)
            check(store.importBackupJson(merged.document.toString())) { "合并结果保存失败" }
            MergeReport(merged.inserted, merged.updated, merged.preserved, 0, merged.vaultChanges,
                message = "冲突 ${merged.conflicts} 项（保留双方副本），删除 ${merged.deleted} 项")
        } catch (e: Exception) {
            Log.e(TAG, "完整快照合并失败", e)
            MergeReport(success = false, message = e.message ?: "合并失败")
        }
    }
    ...
    private fun mergeIdeas(store: DataStore, arr: JSONArray?): Int {
        ...
        for (i in 0 until arr.length()) {
            ...
            if (existing == null) {
                ...
                map[id] = idea
                count++
            } else if (idea.updatedAt > existing.updatedAt) {
                val idx = current.indexOfFirst { it.id == id }
                if (idx != -1) current[idx] = idea
                map[id] = idea
                count++
            }
        }
    ...
    private fun mergeClippings(store: DataStore, arr: JSONArray?): Int {
        ...
        for (i in 0 until arr.length()) {
            ...
            if (existing == null) {
                ...
                map[id] = clip
                count++
            } else if (clip.capturedAt > existing.capturedAt) {
                val idx = current.indexOfFirst { it.id == id }
                if (idx != -1) current[idx] = clip
                map[id] = clip
                count++
            }
        }
