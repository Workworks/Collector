...
object GlobalSearchDialog {
    ...
    fun show(activity: Activity, store: DataStore, onFilterMainList: ((String) -> Unit)? = null) {
        ModernDialogHelper.showInputDialog(
            context = activity,
            title = "全库与 12 馆联合检索",
            subtitle = "支持跨资产主库及全部 12 个专业收纳馆检索：",
            hint = "例如: 电池、茅台、身份证、退烧、三体",
            emoji = "🔍",
            positiveText = "全库检索",
            negativeText = "重置筛选"
        ) { query ->
            ...
            if (q.isEmpty()) {
                onFilterMainList?.invoke("")
                return@showInputDialog
            }

            val results = searchAll(store, q)
            if (results.isEmpty()) {
                Toast.makeText(activity, "🔍 未找到与「$query」相关的记录", Toast.LENGTH_SHORT).show()
